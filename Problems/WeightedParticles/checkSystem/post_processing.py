...
if __name__=="__main__":
    ...
    rho_Dt_fp= np.loadtxt('data/obs/25-11-Rho_pde_dx_e-4_eps-5.out') 
    Jv_pde= np.loadtxt('data/obs/25-11-Jv_pde_dx_e-4_eps-5.out')
    
    sde_rho_sq = np.loadtxt('data/obs/25-11-rho_sq_eps-5.out')
    E_rho = np.loadtxt('data/obs/25-11-E_rho_eps-5.out')
    sq_E_rho  =np.loadtxt('data/obs/25-11-sq_E_rho_eps-5.out')
    
    sde_Jv_sq = np.loadtxt('data/obs/25-11-Jv_sq_eps-5.out')
    E_Jv = np.loadtxt('data/obs/25-11-E_Jv_eps-5.out')   #lots of data, not needed when only interested in the variance
    sq_E_Jv =  np.loadtxt('data/obs/25-11-sq_E_Jv_eps-5.out' )
...
#    
    plot_rho = plt.plot( grid, rho_Dt_fp, label = '$\$')
    #plot_rho =  plt.plot(rho_Dt_sde, label = '$\$')
    plt.ylabel(r' $\mathbf{\rho(x)}$', fontsize = 16)
 #   plt.xlabel('$x$', fontsize = 16)
    plt.savefig('plots/rho_pde.pdf')
    plt.show()
